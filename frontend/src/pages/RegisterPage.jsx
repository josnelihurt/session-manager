...
export function RegisterPage() {
  ...
  const validateInvitation = async (invitationToken) => {
    try {
      ...
      // Set provider from invitation (defaults to 'local' if not specified)
      setProvider(response.provider || 'local')
      // Pre-fill username with email prefix (part before @)
      const usernameFromEmail = response.email.split('@')[0]
