...
# Format .NET projects
for proj in $(find "$PROJECT_ROOT/src" "$PROJECT_ROOT/tests" -name "*.csproj" 2>/dev/null); do
    dotnet format "$proj" --verbosity quiet
done

# Run Roslynator if available
if command -v roslynator &> /dev/null; then
    for proj in $(find "$PROJECT_ROOT/src" "$PROJECT_ROOT/tests" -name "*.csproj" 2>/dev/null); do
        roslynator analyze "$proj" --severity-level warning --ignore-analyzer-results 2>/dev/null || true
    done
fi
